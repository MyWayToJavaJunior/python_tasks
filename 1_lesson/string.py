...
def donuts(count):
    ...
    if count >= 10:
        result = 'Количество пончиков: много'
    else:
        result = 'Количество пончиков: %d' % count
    return result
...
def not_bad(s):
    new_s = s
    if s.find('плох') > s.find('не'):
        new_l = s.replace(' не ', ' хорош ').split(' ')
        new_s = " ".join(new_l[:new_l.index('хорош') + 1])
    return new_s
...
def front_back(a, b):
    ...
    # count_a = len(a) / 2 + len(a) % 2
    # count_b = len(b) / 2 + len(b) % 2
    return '{0}{1}{2}{3}'.format(a[:count_a], b[:count_b], a[count_a:], b[count_b:])
...
def main():
    ...
    test(front_back('Kit', 'Donu'), 'KiDotnu')
