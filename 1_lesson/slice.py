...
def shift(s):
    ...
    return "{0} {1}".format(new_s[-1], " ".join(new_s[0:-1]))
...
def inside(s):
    new_s = s.split(" ")
    i = len(new_s)/2
    return "{0} {1} {2} {3}".format(" ".join(new_s[1:i]), new_s[0], new_s[-1], " ".join(new_s[i:-1]))
...
def main():
    ...
    test(inside('timur eesad sc sa a f rt12 dinara'), 'eesad sc sa timur dinara a f rt12')
