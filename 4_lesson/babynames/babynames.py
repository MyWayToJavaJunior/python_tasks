...
def extract_names(filename):
    ...
    res = {}
    for row_data in name_regexp.findall(s):
        res[row_data[1].strip()] = [x.strip() for x in row_data[2:]]
...
def print_names(babynames):
    ...
    while True:
        user_input = input('Введите один из годов {} или q для выхода: '.format(', '.join(years_list)))
        if user_input == 'q':
            sys.exit(0)
        elif user_input not in years_list:
            continue

        years_index = years_list.index(user_input)

        for k, v in sorted(babynames.items(), key=lambda x: x[0]):
            print('{name:30}\t{value:>6}'.format(name=k, value=v[years_index]))
