...
def max_five():
    ...
    fives = (s[i:i+5] for i in range(len(s) - 4))
    fives_digits = map(lambda x: [int(y) for y in x], fives)
    fives_mul = map(lambda x: reduce(lambda mem, next: mem * next, x), fives_digits)
    return max(fives_mul)
...
def factorial():
    fact_100 = reduce(lambda mem, next: mem * next, range(1, 101))
    return sum(int(x) for x in str(fact_100))
